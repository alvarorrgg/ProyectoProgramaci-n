...
int main(int argc, char *argv[]) {
  ...
  if(argc>2 && argc==4){
  	if(strcmp(argv[2],"-l")==0) { /*Verifica que el tercer argumento es -l*/
      f = fopen(argv[3], "w");
    }
  	else{
  	...
  	return 0;
   	...
   	if(argc==3) { /*Verifica que despues de haber puesto -l, se ha introducido el nombre de un archivo*/
...
int game_loop_init(Game *game, Graphic_engine **gengine, char *file_name){
  if (game_create_from_file(game, file_name) == ERROR) { /*Se crea el juego*/
    fprintf(stderr, "Error while initializing game.\n");
    return 1;
  }

  if ((*gengine = graphic_engine_create()) == NULL) { /*Se crea el motor grafico*/
